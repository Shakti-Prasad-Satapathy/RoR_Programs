...
# 
# @Purpose : practice
# @overview : A program to to list the numbers which are both prime and anagram using queue 
...
=begin
  ...
  #                up from the queue by traversing the queue from starting and add in to a new array
...
# Basic operetions
obj = Using_queue.new(nil)  #object creation

temp = obj.is_prime
for i in 0..(temp.size) - 1
  obj.add_to_queue(temp[i]) # adding elements from temp array to queue bu calling add_to_queue()
end
obj.remove  # calling removing()
