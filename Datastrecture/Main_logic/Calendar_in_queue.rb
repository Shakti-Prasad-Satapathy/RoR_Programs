...
# 
# @Purpose : practice
# @overview : A program to take month and year from user and then print the calendar using queue
...
=begin
# 
# @description : this methode will push the values given by the argument in a queue
# @purpose : adding elements in a queue
# @param {value}, which contains the the value to be entered in the queue
#
=end
  ...
  def add_to_queue(value)
    ...
    current_node.next = Node.new(current_node, value, nil) # initializing the node class
  end
=begin
# 
# @description : this methode will push the values given by the argument in a queue
...
=end
  def display_calendar(day, month, year, days, dayx)
    ...
    for i in 1..temp # for allignment of dats in calendar
      print "   "
    end
    ...
    while current_node.next != nil
      if current_node.value == temp && temp < 10 # to convert 1-01,2-02,....9-09
        print " 0#{current_node.value}"
        temp = temp + 1
      else
        print " #{current_node.value}"
      end
      #Switch to next line after compleation of a week
...
obj = Calendar_queue.new(1) # initializing the current class
for i in 2..(days[month])
  obj.add_to_queue(i)     # adding values (dates in the queue)
end
